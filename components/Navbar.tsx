...
const Navbar: React.FC = () => {
  ...
  // LinkedIn-style nav items with icons
  // LinkedIn-style nav items with icons
  const navItems = [
    { label: 'Home', path: '/feed', icon: <Icon icon="solar:home-angle-linear" width="24" height="24" /> },
    { label: 'Network', path: '/network', icon: <Icon icon="solar:users-group-rounded-linear" width="24" height="24" /> },
    { label: 'Jobs', path: '/jobs', icon: <Icon icon="solar:case-round-linear" width="24" height="24" /> },
    { label: 'Notices', path: '/notices', icon: <Icon icon="solar:chat-line-linear" width="24" height="24" /> },
    { label: 'Groups', path: '/groups', icon: <Icon icon="solar:users-group-two-rounded-linear" width="24" height="24" /> },
    { label: 'Mentorship', path: '/mentorship', icon: <Icon icon="solar:medal-star-linear" width="24" height="24" /> },
    { label: 'Events', path: '/events', icon: <Icon icon="solar:calendar-linear" width="24" height="24" /> },
  ];
  ...
  return (
    <nav className="sticky top-0 z-50 glass border-b border-white/20 px-4 md:px-12 py-4 flex items-center justify-between backdrop-blur-xl">
      <div className="flex items-center gap-4">
        ...
        >
          {isMobileMenuOpen ? <Icon icon="solar:close-circle-linear" width="24" height="24" /> : <Icon icon="solar:hamburger-menu-linear" width="24" height="24" />}
        </button>
      ...
      {/* Nav Links - Desktop */}
      {/* Centered Nav Links - Desktop (LinkedIn Style) */}
      <div className="hidden lg:flex items-center gap-2 xl:gap-6 flex-grow justify-center">
        {user && navItems.map((item) => (
          <Link
            key={item.label}
            to={item.path}
            className={`flex flex-col items-center justify-center gap-1 px-3 py-2 min-w-[70px] transition-all relative group ${location.pathname === item.path ? 'text-slate-900' : 'text-slate-500 hover:text-slate-900'
              }`}
          >
            <div className={`relative ${location.pathname === item.path ? 'text-slate-900' : 'text-slate-500 group-hover:text-slate-900'}`}>
              {item.icon}
              {/* Optional: Add notification dots here if needed */}
            </div>
            <span className={`text-[10px] font-medium tracking-wide hidden xl:block ${location.pathname === item.path ? 'font-bold' : ''}`}>
              {item.label}
            </span>
      ...
      <div className="flex items-center gap-2 md:gap-4">
        {/* Simple Search Input - Only if logged in */}
        {user && (
          <div className="relative" ref={searchRef}>
            <div className={`flex items-center bg-slate-100/80 rounded-2xl px-3 py-2 transition-all ${isSearchOpen ? 'w-48 md:w-64 ring-2 ring-blue-100 bg-white' : 'w-10 md:w-48'}`}>
              <Icon icon="solar:magnifer-linear" width="18" height="18" className="text-slate-400 shrink-0" />
              <input
                type="text"
                placeholder="Search..."
                        ...
                        <div>
                          ...
                          {results.jobs.map(j => (
                            <div key={j.id} onClick={() => handleResultClick(`/jobs`)} className="p-3 hover:bg-slate-50 rounded-xl cursor-pointer flex items-center gap-3 transition-colors">
                              <div className="w-8 h-8 bg-blue-50 text-blue-600 rounded-lg flex items-center justify-center"><Icon icon="solar:case-round-linear" width="16" height="16" /></div>
                        ...
                        <div>
                          ...
                          {results.events.map(e => (
                            <div key={e.id} onClick={() => handleResultClick(`/events`)} className="p-3 hover:bg-slate-50 rounded-xl cursor-pointer flex items-center gap-3 transition-colors">
                              <div className="w-8 h-8 bg-orange-50 text-orange-600 rounded-lg flex items-center justify-center"><Icon icon="solar:calendar-linear" width="16" height="16" /></div>
                ...
                <div className="max-h-[400px] overflow-y-auto">
                  {!user ? (
                    <div className="p-10 text-center">
                      <Icon icon="solar:stars-minimalistic-bold" width="32" height="32" className="mx-auto text-blue-400 mb-4" />
                    ...
                    <div className="p-10 text-center">
                      <Icon icon="solar:bell-off-linear" width="32" height="32" className="mx-auto text-slate-300 mb-4" />
                    ...
                    notifications.map(n => (
                      ...
                      >
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${n.type === 'job' ? 'bg-emerald-50 text-emerald-600' :
                          n.type === 'event' ? 'bg-orange-50 text-orange-600' :
                            'bg-blue-50 text-blue-600'
                          }`}>
                          {n.type === 'job' ? <Icon icon="solar:case-round-linear" width="18" height="18" /> : n.type === 'event' ? <Icon icon="solar:calendar-linear" width="18" height="18" /> : n.type === 'post' ? <Icon icon="solar:document-text-linear" width="18" height="18" /> : <Icon icon="solar:user-check-linear" width="18" height="18" />}
                        </div>
            ...
            <div className="space-y-6 flex-grow">
                ...
                <div className="space-y-1">
                  {user && navItems.map((item) => (
                    <Link
                      ...
                      className={`flex items-center justify-between p-4 rounded-2xl font-black text-lg transition-all ${location.pathname === item.path
                        ? 'bg-blue-50 text-blue-600'
                        : 'text-slate-900 hover:bg-slate-50'
                        }`}
                    >
                      {item.label}
                      <Icon icon="solar:alt-arrow-right-linear" width="20" height="20" className={location.pathname === item.path ? 'opacity-100' : 'opacity-20'} />
                    </Link>
                  ))}
                  {user?.role === 'Admin' && (
                    <Link
                    ...
                    >
                      <Icon icon="solar:widget-linear" width="20" height="20" /> Admin Dashboard
                    </Link>
                  )}
                </div>
              </div>
              ...
              {user && (
                <div className="pt-2">
                  ...
                  >
                    <Icon icon="solar:logout-2-linear" width="20" height="20" /> Sign Out
                  </button>
                </div>
              )}
            </div>
